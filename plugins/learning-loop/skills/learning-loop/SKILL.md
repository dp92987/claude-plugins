...
**Interactive runs delegate steps 1–5 to the `learning-extractor` subagent** (Agent tool, subagent_type `learning-extractor`) so extraction runs on a cheap model regardless of what the session itself runs on. Pass it: the transcript path, plus the absolute paths to `scripts/find-corrections.sh`, `references/rule-format.md` (both relative to this SKILL.md's directory), and the memory directory. The agent defaults to sonnet; if `~/.claude/learning-loop-memory/model` holds `haiku`, `sonnet`, or `opus`, pass that as the Agent `model` override (a full model ID in that file only applies to headless runs — skip the override then). For "this session", the transcript is the most recently modified `~/.claude/projects/<cwd-slug>/*.jsonl`, where the slug is the cwd with `/` and `.` replaced by `-` (e.g. `-data-projects-github-com-dp92987-claude-plugins`); before trusting it, grep it for a distinctive phrase from the current conversation — if it doesn't match (concurrent session in the same repo), fall back to extracting in-context. Headless runs have no Agent tool and already run on the configured model — extract in-context. Either way, continue with steps 6–9 yourself on the returned candidates.
